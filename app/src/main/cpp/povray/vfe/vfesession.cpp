...
vfeSession::vfeSession(int id)
{
  ...
  m_DisplayCreator = std::bind(&vfe::vfeSession::DefaultDisplayCreator, this, _1, _2, _3, _4, _5);
  Reset();
}
...
void vfeSession::AppendAnimationStatus (int Frame, int Total, const UCS2String& Filename)
{
  ...
  m_StatusLineMessage = string("Rendering frame ") + std::to_string(Frame) + " of " + std::to_string(Total);
  if (m_MaxStatusMessages != -1)
    while (m_StatusQueue.size() > m_MaxStatusMessages)
      m_StatusQueue.pop();
...
// Returns a std::string giving a short english description of the error code
...
bool vfeSession::Pause()
{
  ...
  static const std::chrono::seconds period(3);

  m_RequestFlag = rqPauseRequest;
  if (m_RequestEvent.wait_for(lock, period) == std::cv_status::timeout)
  {
    m_RequestFlag = rqNoRequest;
...
int vfeSession::Initialize(vfeDestInfo *Dest, vfeAuthInfo *Auth)
{
...
#ifdef _DEBUG
  static const std::chrono::seconds period(120);
#else
  static const std::chrono::seconds period(3);
#endif
  ...
  if (m_BackendState == kUnknown && m_InitializeEvent.wait_for(lock, period) == std::cv_status::timeout)
  {
    m_WorkerThreadShutdownRequest = true ;
    m_Initialized = false ;
