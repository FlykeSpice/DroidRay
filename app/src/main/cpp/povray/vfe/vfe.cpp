...
void vfeParserMessageHandler::Progress(Console *Con, POVMS_Object& Obj, bool verbose)
{
  ...
  switch(Obj.GetType(kPOVMSObjectClassID))
  {
    ...
    {
      std::snprintf (buf, 30, "Parsing %lldK tokens", Obj.GetLong (kPOVAttrib_CurrentTokenCount) / 1000);
      m_Session->AppendStatusMessage (string(buf));
      break;
    }
    case kPOVObjectClass_BoundingProgress:
    {
      std::snprintf (buf, 30, "Constructed %lldK BSP nodes", Obj.GetLong (kPOVAttrib_CurrentNodeCount) / 1000);
      m_Session->AppendStatusMessage (string(buf));
      break;
    }
  }
...
void vfeParserMessageHandler::Warning(Console *Con, POVMS_Object& Obj, bool conout)
{
  ...
  if (!d.File.empty() && (d.Line > 0))
  {
    ...
    std::snprintf (buf, 200, "File '%s' line %d: %s", d.File.c_str(), d.Line, d.Message.c_str());
    if (m_Session->m_OptimizeForConsoleOutput == false)
      m_Session->AppendStatusMessage (buf) ;
    else if (conout)
      Con->puts (buf) ;
  }
  else
...
void vfeParserMessageHandler::Error(Console *Con, POVMS_Object& Obj, bool conout)
{
  ...
  if (!d.Message.empty())
  {
    ...
    {
      char buf[300];
      std::sprintf (buf, "File '%s' line %d: %s", d.File.c_str(), d.Line, d.Message.c_str());
  ...
  {
    char buf[300];
    std::sprintf(buf, "Parse error in file '%s' at line %d", d.File.c_str(), d.Line);
...
void vfeRenderMessageHandler::Progress(Console *Con, POVMS_Object& Obj, bool verbose)
{
  ...
  switch (Obj.GetType(kPOVMSObjectClassID))
  {
    case kPOVObjectClass_PhotonProgress:
    {
      ...
      m_Session->AppendStatusMessage (string ("Photon count ") + std::to_string(cpc), 250) ;
      break;
    }
    case kPOVObjectClass_RadiosityProgress:
    {
      ...
      std::snprintf(buf, 100, "Performing radiosity pretrace: %d of %d pixels (%d%%)", cc, pc, percent);
      m_Session->AppendStatusMessage (buf, 250) ;
      break;
    }
	...
	{
	  std::snprintf(buf, 100, "Rendered %u of %u pixels (%d%%)", cc, pc, percent);
      	  m_Session->AppendStatusMessage (buf, 250) ;
	}
      }
      else
      {
        ...
        if (verbose == true || m_Session->m_OptimizeForConsoleOutput == false)
	{
	  std::snprintf(buf, 100, "Rendered %g frames over %g seconds (%g FPS)", frames, elapsed, fps);
          m_Session->AppendStatusMessage (buf, 250) ;
	}
      }
      break;
    ...
    case kPausedParsing:
      switch(renderFrontend.GetSceneState(sceneId))
      {
        ...
        case SceneData::Scene_Ready:
          ...
          try { viewId = renderFrontend.CreateView(sceneId, options, imageProcessing, std::bind(&vfe::VirtualFrontEnd::CreateDisplay, this, _1, _2, _3)); }
          catch(pov_base::Exception& e)
          {
            m_Session->SetFailed();
            m_Session->AppendErrorMessage (e.what()) ;
