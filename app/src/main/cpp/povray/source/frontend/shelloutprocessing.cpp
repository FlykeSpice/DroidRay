...
void ShelloutAction::ExpandParameters(const string& scene, const string& ofn, unsigned int w, unsigned int h, float clock, unsigned int frame)
{
	...
	for (const char *cmd = rawParameters.c_str(); *cmd; cmd++)
	{
		...
		switch (*++cmd)
		{
			...
			case 'n':
				std::sprintf(buf, "%u", frame);
				parameters.append(buf);
				break;

			case 'k':
				std::sprintf(buf, "%#.6f", clock);
				parameters.append(buf);
				break;
...
bool ShelloutProcessing::ExtractCommand(const string& src, string& command, string& parameters) const
{
	...
	if (command.size() > 1)
	{
		...
		if ((ch1 == '\'' || ch1 == '"') && ch1 == ch2)
		{
			command = pov_trim_copy(command.substr(1, command.size() - 2));
			if (command.empty())
				return false;
		}
	}

	parameters = pov_trim_copy(string(s));
	return true;
}
...
bool ShelloutProcessing::HandleProcessEvent(shelloutEvent event, bool internalCall)
{
	...
	if (!CommandPermitted(sh->Command(), sh->Parameters()))
	{
		...
		char buf[30];
		std::sprintf(buf, "Execution of shellout '%s' prohibited", sh->Command().c_str());

		throw POV_EXCEPTION(kCannotOpenFileErr, string(buf));
	}
					...
					switch (action)
					{
						...
						case ShelloutAction::skipAll:
							...
							else
							{
								...
								std::snprintf(buf, 20, "Skip frame %d", frameNo+1);
...
{
	string copy = cancelFormat;
	string::size_type pos = 0;

	const std::string messages[] = {cancelPhase, std::to_string(exitCode), ExitDesc(),  ExitDesc(), cancelReason, cancelCommand, cancelParameters, std::to_string(cancelReturn), cancelOutput};
	char fmt[] = {'%', '1', '%', '\0'};

	for (int i=0; i<9; i++)
	{
		fmt[1] = '1'+i;
		while ((pos = copy.find(fmt)) != std::string::npos)
			copy.replace(pos, 3, messages[i]);
	}
	return copy;
}
...
{
	string copy = skipFormat;
	string::size_type pos = 0;

	const std::string messages[] = {skipPhase, skipReason, skipCommand, skipParameters, std::to_string(skipReturn), skipOutput};
	char fmt[] = {'%', '1', '%', '\0'};
