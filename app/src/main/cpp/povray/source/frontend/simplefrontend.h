...
class SimpleFrontend
{
	public:
		SimpleFrontend(POVMSContext ctx, POVMSAddress addr, POVMS_Object& msg,
		               std::function<Console *()> cfn,
		               std::function<Display *(unsigned int, unsigned int, GammaCurvePtr)> dfn,
	...
	private:
		...
		std::function<Console *()> createConsole;
		std::function<Display *(unsigned int, unsigned int, GammaCurvePtr)> createDisplay;
};
