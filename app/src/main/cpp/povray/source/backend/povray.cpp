...
std::thread *POV_MainThread = NULL;
 ...
 */
void MainThreadFunction(const std::function<void ()>& completion);
...
void MainThreadFunction(const std::function<void ()>& threadExit)
{
	try
	{
...
std::thread *povray_init(const std::function<void ()>& threadExit, POVMSAddress *addr)
{
	using namespace pov;
	...
	if(POV_MainThread == NULL)
	{
		...
		POV_MainThread = Task::NewBoostThread(std::bind(&MainThreadFunction, threadExit), POV_THREAD_STACK_SIZE);
...
//	argv = nargv;
	...
	else
	{
		...
		SimpleFrontend<ParserMessageHandler, FileMessageHandler, RenderMessageHandler, ImageMessageHandler>
		               ...
		               std::bind(CreateDefaultConsole), std::bind(CreateDefaultDisplay, _1, _2, _3));
