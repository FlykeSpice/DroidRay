...
struct Crackle_Cell_Hasher
{
	std::size_t operator()(const Crackle_Cell_Coord& p) const  noexcept
	{
//Taken from boost::hash_combine implementation
#define hash_combine(seed, value) seed ^= value + 0x9e3779b9 + (seed << 6) + (seed >> 2)
		std::size_t seed = 0;
...
typedef std::unordered_map<Crackle_Cell_Coord, Crackle_Cache_Entry, Crackle_Cell_Hasher> Crackle_Cache_Type;
