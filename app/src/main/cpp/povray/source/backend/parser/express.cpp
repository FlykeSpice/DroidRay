	...
	EXPECT
		CASE (FLOAT_FUNCT_TOKEN)
			...
			switch(Token.Function_Id)
			{
				...
				case NOW_TOKEN:
					{
						static std::tm y2k = {.tm_year = 2000-1900, .tm_mday = 1};

						auto now = std::chrono::system_clock::now();
						auto y2ktime = std::chrono::system_clock::from_time_t(std::mktime(&y2k));

						Val = std::chrono::duration_cast<std::chrono::microseconds>(now-y2ktime).count() * (1.0e-6) / (24*60*60);
					}
					break;
			}
			for (i=0; i < *Terms; i++)
