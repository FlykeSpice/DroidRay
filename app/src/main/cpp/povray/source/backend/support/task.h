...
class Task
{
	...
	public:
		...
		Task(TaskData *td, const std::function<void (Exception&)>& f);
		virtual ~Task();
		...
		void Start(const std::function<void ()>& completion);
		void RequestStop();
		void Stop();
		void Pause();
		void Resume();
		...
		inline static std::thread* NewBoostThread(CALLABLE_T func, int stackSize)
		{
	...
	private:
		...
		inline static DECLARE_THREAD_LOCAL_PTR(TaskData, taskDataPtr);
		/// task fatal error handler
		std::function<void (Exception&)> fatalErrorHandler;
		/// stop request flag
		...
		void TaskThread(const std::function<void ()>& completion);
