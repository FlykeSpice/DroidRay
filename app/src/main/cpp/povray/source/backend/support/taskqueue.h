...
class TaskQueue
{
		class TaskEntry
		{
			public:
				...
				TaskEntry(const std::function<void (TaskQueue&)>& f) : entryType(kFunction), fn(f) { }
				~TaskEntry() { }

				shared_ptr<Task> GetTask() { return task; }
				POVMS_Message& GetMessage() { return msg; }
				std::function<void (TaskQueue&)>& GetFunction() { return fn; }
			...
			private:
				...
				POVMS_Message msg;
				std::function<void (TaskQueue&)> fn;
		};
	public:
		TaskQueue();
		~TaskQueue();
		...
		void AppendFunction(const std::function<void (TaskQueue&)>& fn);
