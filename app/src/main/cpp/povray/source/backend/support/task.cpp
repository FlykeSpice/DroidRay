...
Task::Task(TaskData *td, const std::function<void (Exception&)>& f) :
	taskData(td),
	fatalErrorHandler(f),
	stopRequested(false),
	paused(false),
...
void Task::Start(const std::function<void ()>& completion)
{
	if((done == false) && (taskThread == NULL))
		taskThread = NewBoostThread(std::bind(&Task::TaskThread, this, completion), POV_THREAD_STACK_SIZE);
}
...
void Task::TaskThread(const std::function<void ()>& completion)
{
	int result;
