...
void Scene::StartParser(POVMS_Object& parseOptions)
{
	...
	if(parserControlThread == NULL)
		parserControlThread = Task::NewBoostThread(std::bind(&Scene::ParserControlThread, this), POV_THREAD_STACK_SIZE);
	else
		return;
	...
	// send statistics
	parserTasks.AppendFunction(std::bind(&Scene::SendStatistics, this, _1));

	// send done message and compatibility data
	parserTasks.AppendFunction(std::bind(&Scene::SendDoneMessage, this, _1));
}
