...
{
	std::scoped_lock lock(nextBlockMutex);
...
{
	std::scoped_lock lock(setDataMutex);
...
void View::StartRender(POVMS_Object& renderOptions)
{
	...
	if(renderControlThread == NULL)
		renderControlThread = Task::NewBoostThread(std::bind(&View::RenderControlThread, this), POV_THREAD_STACK_SIZE);
	...
	if (viewData.renderBlockStep > 0)
	{
		...
		if (!viewData.renderBlockStep)
			viewData.renderBlockStep = (viewData.blockWidth*viewData.blockHeight);
		while(std::gcd((long)viewData.renderBlockStep,(long)(viewData.blockWidth*viewData.blockHeight)) > 1)
	...
	*/
	...
	if(viewData.GetSceneData()->radiositySettings.radiosityEnabled)
	{
		...
		if (highReproducibility)
		{
			...
			for(int step = RadiosityFunction::PRETRACE_FIRST; step < RadiosityFunction::PRETRACE_FIRST + steps; step ++)
			{
				...
				renderTasks.AppendFunction(std::bind(&View::SetNextRectangle, this, _1, blockskiplist, nextblock));
	...
	renderTasks.AppendFunction(std::bind(&View::DispatchShutdownMessages, this, _1));

	// wait for shutdown messages to be sent
	renderTasks.AppendSync();

	// send statistics
	renderTasks.AppendFunction(std::bind(&View::SendStatistics, this, _1));
...
const Camera *RTRData::CompletedFrame()
{
	...
	static std::chrono::seconds period(3);
	...
	if (event.wait_for(lock, period) == std::cv_status::timeout)
