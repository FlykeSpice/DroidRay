...
RadiosityCache::~RadiosityCache()
{
	...
	{ // mutex scope
		std::scoped_lock lockTree(octree.treeMutex);
		std::scoped_lock lockBlock(octree.blockMutex);
		if (octree.root != NULL)
			ot_free_tree(&octree.root);
	}

	{ // mutex scope
		std::scoped_lock lock(blockPoolsMutex);
...
{
	std::scoped_lock lock(octree.blockMutex);
