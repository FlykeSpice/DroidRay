                                  ...
                                  const Vector3d& rawnormal, const Ray& ray, COLC weight, Intersection& isect, TraceTicket& ticket)
{
	...
	bool tir_occured;
	std::unique_ptr<PhotonGatherer> surfacePhotonGatherer;
	...
	if(sceneData->photonSettings.photonsEnabled && sceneData->surfacePhotonMap.numPhotons > 0)
		surfacePhotonGatherer = std::make_unique<PhotonGatherer>(&sceneData->surfacePhotonMap, sceneData->photonSettings);
